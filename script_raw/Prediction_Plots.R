...
- Climate_predicted <- read_csv(paste("./data/data_proj/output/", namelist_RCMs[[t]],"/Climate_predicted.csv", sep="")) 
...
paste("./figures/figures_exploratory/Proj/", namelist_RCMs[[t]] , "/DeskriptiveStats_diff2021.txt", sep="")

Plots:
paste("./figures/figures_exploratory/Proj/", namelist_RCMs[[t]],"/plot_mean_diff_YAnomaly_", nameList_climate[[s]],".pdf", sep="")
...
plot_mean_1971_list <- plot_mean_diff2021_list <-plot_mean_diff2070_list <- plot_sd_1971_list <- plot_sd_diff2021_list <- plot_sd_diff2070_list <- 
  nameList_climate_store 
...
for (t in seq_along(namelist_RCMs)){
  ...
  dir.create(paste("./figures/figures_exploratory/Proj/", namelist_RCMs[[t]] ,sep=""), showWarnings = FALSE)
  ...
  Climate_predicted
  # summary(Climate_predicted)
  
  ## Check predicted anomaly values ##
  Climate_predicted %>% select(contains("sMA")) %>% summary()
  
  #########################################################################
  #### Generate list with data.frame container for each climate period ####
  Climate_predicted_summaries_list <- list(Climate_predicted_summaries_1979 = data.frame(), 
                                           Climate_predicted_summaries_2021 = data.frame(),
                                           Climate_predicted_summaries_2070 = data.frame())
  
  ######################################
  #### List to store sf.data.frames ####
  Climate_predicted_summaries_sf_list <- list(Climate_predicted_summaries_sf_1979 = data.frame(), 
                                              Climate_predicted_summaries_sf_2021 = data.frame(),
                                              Climate_predicted_summaries_sf_2070 = data.frame())
  ...
  #####################################################################################################################
  for (r in 1:3){
    Climate_predicted_summaries_list[[r]]  <- 
      Climate_predicted  %>% 
  ...
  Climate_predicted_summaries_list[[1]] %>% select(contains("_mean")) %>% summary()
  Climate_predicted_summaries_list[[2]] %>% select(contains("_mean")) %>% summary()
  Climate_predicted_summaries_list[[3]] %>% select(contains("_mean")) %>% summary()

  ###########################################################
  #### Compare Mean values for the three climate periods ####
  Climate_predicted_summaries_list[[1]] %>% select(contains("_sd")) %>% summary()
  Climate_predicted_summaries_list[[2]] %>% select(contains("_sd")) %>% summary()
  Climate_predicted_summaries_list[[3]] %>% select(contains("_sd")) %>% summary()

  ##################################################################################################################################
  #### Create differences in between the reference climate period (1971 - 2000) and the projections (2021 - 2050, 2070 - 2099) ####
  ################################################################################################################################
  Climate_predicted_summaries_diff2021 <- as_tibble(Climate_predicted_summaries_list[[2]] - Climate_predicted_summaries_list[[1]])
  Climate_predicted_summaries_diff2070 <- as_tibble(Climate_predicted_summaries_list[[3]] - Climate_predicted_summaries_list[[1]])
  
  #### Add comIds to allow for merging ####
  Climate_predicted_summaries_diff2021$comId <- Climate_predicted_summaries_list[[2]]$comId
  ...
  # #### Export summary statistics ####
  # ##################################
  stargazer(as.data.frame(Climate_predicted_summaries_diff2021), type = "text", title="Descriptive statistics", digits=3,
            out = paste("./figures/figures_exploratory/Proj/", namelist_RCMs[[t]] , "/DeskriptiveStats_diff2021_mean_sd.txt", sep=""))
  stargazer(as.data.frame(Climate_predicted_summaries_diff2070), type = "text", title="Descriptive statistics", digits=3,
            out = paste("./figures/figures_exploratory/Proj/", namelist_RCMs[[t]] , "/DeskriptiveStats_diff2070_mean_sd..txt", sep=""))
  stargazer(as.data.frame(Climate_predicted_summaries_list[[1]]), type = "text", title="Descriptive statistics", digits=3,
            out = paste("./figures/figures_exploratory/Proj/", namelist_RCMs[[t]] , "/DeskriptiveStats_1971_mean_sd..txt", sep=""))
  stargazer(as.data.frame(Climate_predicted_summaries_list[[2]]), type = "text", title="Descriptive statistics", digits=3,
            out = paste("./figures/figures_exploratory/Proj/", namelist_RCMs[[t]] , "/DeskriptiveStats_2021_mean_sd.txt", sep=""))
  stargazer(as.data.frame(Climate_predicted_summaries_list[[3]]), type = "text", title="Descriptive statistics", digits=3,
            out = paste("./figures/figures_exploratory/Proj/", namelist_RCMs[[t]] , "/DeskriptiveStats_2070_mean_sd..txt", sep=""))
  # 
  ##############################################################################################################################################################################
  #########################################################################
  ...
  for (s in seq_along(nameList_climate))
    ...
    # ###################################################
    # #### Plot mean of sMA for each climate period ####
    # #################################################
    # 
    # summary(Climate_predicted_summaries_sf_list [[1]]$sMA_lm.fit_SMI_6_Jun_Aug_anomaly_demean_mean)
    # summary(Climate_predicted_summaries_sf_list [[2]]$sMA_lm.fit_SMI_6_Jun_Aug_anomaly_demean_mean)
    # summary(Climate_predicted_summaries_sf_list [[3]]$sMA_lm.fit_SMI_6_Jun_Aug_anomaly_demean_mean)
    # 
    # myPalette <- colorRampPalette((brewer.pal(11, "RdYlGn")))
    # sc <- scale_fill_gradientn("Yield Anomaly", colours = myPalette(100), limits=c(-150, 150))
    # 
    # ## Plot Mean Y: 1971 - 2000 ##
    # plot_mean_1971_Y <-
    #   ggplot(Climate_predicted_summaries_sf_list [[1]]) +
    ...
    # plot_mean_2021_Y <-
    #   ggplot(Climate_predicted_summaries_sf_list [[2]]) +
    #   geom_sf(data=vg2500_krs, fill="gray", color="white")  +
    #   geom_sf(aes_string(fill = nameList_climate_sMA_mean[[s]] )) +
    ...
    # ## Plot Mean: 2070 - 2099  ##
    # plot_mean_2070_Y <-
    #   ggplot(Climate_predicted_summaries_sf_list [[3]]) +
    #   geom_sf(data=vg2500_krs, fill="gray", color="white")  +
    #   geom_sf(aes_string(fill = nameList_climate_sMA_mean[[s]] )) +
    ...
    # 
    # ggsave(paste("./figures/figures_exploratory/Proj/", namelist_RCMs[[t]],"/plot_mean_YAnonmaly_", nameList_climate[[s]],".pdf", sep=""), 
    ...
    # 
    # ## Define colorRamp ##
    # str(Climate_predicted_summaries_sf_list,1)
    # summary(Climate_predicted_summaries_sf_list [[1]]$sMA_lm.fit_SMI_6_Jun_Aug_anomaly_demean_sd)
    # summary(Climate_predicted_summaries_sf_list [[2]]$sMA_lm.fit_SMI_6_Jun_Aug_anomaly_demean_sd)
    # summary(Climate_predicted_summaries_sf_list [[3]]$sMA_lm.fit_SMI_6_Jun_Aug_anomaly_demean_sd)
    # 
    # 
    ...
    #                           top=textGrob(paste(namelist_RCMs[[t]], nameList_climate[[s]], sep=" & " ),gp=gpar(fontsize=30))) 
    # 
    # ggsave(paste("./figures/figures_exploratory/Proj/", namelist_RCMs[[t]],"/plot_sd_YAnonmaly_", nameList_climate[[s]],".pdf", sep=""), 
    ...
    Climate_predicted_summaries_diff2021_sf %>% select(contains("sMA")) %>% select(contains("_mean")) %>% summary()
