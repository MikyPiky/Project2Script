...
## Caveat ##
Because of Singularity Isses -> Always Second Best Modell
...
sd(Yield_Covariates_SM_Jul$Prec, na.rm=T)
summary(Yield_Covariates_SM_Jul$Prec, na.rm=T)
hist(Yield_Covariates_SM_Jul$Prec, na.rm=T)

sd(Yield_Covariates_SM_Jul$Tavg, na.rm=T)
summary(Yield_Covariates_SM_Jul$Tavg, na.rm=T)
hist(Yield_Covariates_SM_Jul$Tavg, na.rm=T)
...
# Scale Covariates ##
####################
head(Yield_Covariates_SM_Jul)[,8:length(Yield_Covariates_SM_Jul)-1]
Yield_Covariates_SM_Jul_norm <- Yield_Covariates_SM_Jul[,9:length(Yield_Covariates_SM_Jul)-1]
head(Yield_Covariates_SM_Jul_norm)
scale <- scale(Yield_Covariates_SM_Jul_norm)
summary(Yield_Covariates_SM_Jul_norm)
summary(scale)
Yield_Covariates_SM_Jul[,9:length(Yield_Covariates_SM_Jul)-1] <- scale
...
if ( x > 6)
{
  ...
  model_best <- print("Best Formula - Second Best BIC becaus of Singularity")
  model_best 
...
if (x > 6)
{
  ...
  #############################################################
  ## If difference is smaller than 6, take second best model ##
  model_standard <- print("Standard Formula (Standard Configuration) - Second Best BIC because of Singularity")
  model_standard
...
####################
## PLM Ergebnisse ##
plm.fit_SM_bestStandard_Jul <- plm(formula = update(bestStandard_formula, .~. - dummy(comId)),  data = Yield_Covariates_SM_Jul,  
                                   effect="individual", model=("within"), index = c("comId","year"))
...
stargazer(plm.fit_SM_bestStandard_Jul, plm.fit_SM_bestStandard_Jul, plm.fit_SM_bestStandard_Jul, plm.fit_SM_bestStandard_Jul, plm.fit_SM_bestStandard_Jul,plm.fit_SM_bestStandard_Jul,
          se = se,   
          dep.var.caption  = "Model with smallest BIC (Standard Configuration) - July",
          dep.var.labels = "log(Silomaize)",
          style="default",
          model.numbers = FALSE,
          column.labels = labels1,          
...
if ( x > 6)
{
  ...
  #############################################################
  ## If difference is smaller than 6, take second best model ##
  model_bestSMI <- print("Best Formula (with SMI) - Second Best BIC because of Singularity")
  model_bestSMI
  ## Look at second best model ##
...
if (x > 6)
{
  ...
  #############################################################
  ## If difference is smaller than 6, take second best model ##
  model_noPet <- print("Best Formula (no PET)  -  Second Best BIC because of Singularity")
  model_noPet
...
stargazer(plm.fit_SM_noPet_Jul, plm.fit_SM_noPet_Jul, plm.fit_SM_noPet_Jul, plm.fit_SM_noPet_Jul, plm.fit_SM_noPet_Jul,plm.fit_SM_noPet_Jul,
          se = se,   
          dep.var.caption  = "Model with smallest BIC (no PET) - July",
          dep.var.labels = "log(Silomaize)",
          style="default",
          model.numbers = FALSE,
          column.labels = labels1,          
...
########################################
## Model only employing Tavg ##
formula_Jul_sm_detrendlog_Tavg 
update(formula_Jul_sm_detrendlog_Tavg , .~. - dummy(comId))
plm.fit_SM_BestStdFull_Jul_onlyTemp <- plm(update(formula_Jul_sm_detrendlog_Tavg , .~. - dummy(comId)),  data = Yield_Covariates_SM_Jul,  effect="individual", model=("within"), index = c("comId","year"))

summary(plm.fit_SM_BestStdFull_Jul_onlyTemp )

cov2_SM_BestStdFull_Jul__onlyTemp    <- vcovSCC(plm.fit_SM_BestStdFull_Jul_onlyTemp, method = "arellano",type = "HC0")
DK.se_SM_BestStdFull_Jul__onlyTemp    <- sqrt(diag(cov2_SM_BestStdFull_Jul__onlyTemp ))
...
# ######################################################
# #### Try to make ceteris paribus confidence bands ####
# 
# plm.fit_SM_BestStdFull_Jul <- plm(formula_Jul_sm_detrendlog_SMIPrecTavg,  data = Yield_Covariates_SM_Jul,  effect="individual", model=("within"), index = c("comId","year"))
# summary(plm.fit_SM_BestStdFull_Jul)
# 
# summary(Yield_Covariates_SM_Jul)
# 
# lm.fit_SM_BestStdFull_Jul <- lm(update(formula_Jul_sm_detrendlog_SMIPrecTavg , .~. + dummy(comId)),  data = Yield_Covariates_SM_Jul)
# summary(lm.fit_SM_BestStdFull_Jul)
# 
# newdata = Yield_Covariates_SM_Jul
# 
# newdata$SMI_GDM <- as.factor(0)
# 
# head(dummy(Yield_Covariates_SM_Jul$SMI_GDM, c("severe drought", 
#                  "moderate drought", "abnormally dry", "abnormally wet", "abundantly wet", 
#                  "severely wet")) )
# 
# SMI.dummy  <- dummy(Yield_Covariates_SM_Jul$SMI_GDM, c("severe drought", 
#                                              "moderate drought", "abnormally dry", "abnormally wet", "abundantly wet", "severely wet")) 
# str(SMI.dummy )
# head(SMI.dummy )
# 
# head(data.frame(SMI.dummy ))
# SMI.dummy <- data.frame(SMI.dummy )
# names(SMI.dummy)
# 
# old.data <- Yield_Covariates_SM_Jul
# 
# head(old.data)
# old.data$SMI_GDM <- NULL
# 
# old.data.dummy <- cbind(old.data,SMI.dummy )
# 
# update(formula_Jul_sm_detrendlog_SMIPrecTavg, .~. + dummy(comId))
# 
# r
# lm.fit_SM_BestStdFull_Jul_old.data.dummy  <- lm(siloMaize_logtrend ~ poly(Prec, degree[r, 1], raw = T) + poly(Tavg, degree[r, 2], raw = T) + 
#                                   severe.drought+moderate.drought+abnormally.dry+abnormally.wet+abundantly.wet+severely.wet
#                                 + dummy(comId),  data = old.data.dummy)
# summary(lm.fit_SM_BestStdFull_Jul_old.data.dummy)
# summary(lm.fit_SM_BestStdFull_Jul)
# ' Die Ergebnisse sind die gleichen. '
# 

# ## Make a new data.frame only considering tavg
# 
# new.data.dummy <- old.data.dummy
# new.data.dummy$severe.drought <- 0
# new.data.dummy$moderate.drought <- 0
# new.data.dummy$abnormally.dry <- 0
# new.data.dummy$abnormally.wet <- 0
# new.data.dummy$abundantly.wet <- 0
# new.data.dummy$severely.wet <- 0
# 
# head(new.data.dummy[order(new.data.dummy$Prec), ])
# head(new.data.dummy)
# summary(new.data.dummy)
# 
# new.data.dummy2 <- new.data.dummy[order(new.data.dummy$Prec),]
# 
# dim(new.data.dummy)
# dim(new.data.dummy2)
# 
# predict.model <-predict(lm.fit_SM_BestStdFull_Jul_old.data.dummy, new.data.dummy2, interval = "confidence")
# 
# head(predict.model)
# str(predict.model)
# 
...
# 
# 
# plot(new.data.dummy2$Prec, predict.model[,1] )
# lines(new.data.dummy2$Prec, predict.model[,2])
# 
# require(graphics)
# 
# ## Predictions
