...
# Maize_meteo <- inner_join(YieldMeteo, Maize_meteo_select, by=c("year","comId"))
# names(Maize_meteo )
...
Maize_meteo_select <- Maize_meteo %>% select(matches("May|Jun|Jul|Aug|Sep|Oct"))

####################################
#### Data with T instead of PET ####
x <- Maize_meteo_select %>% select(SMI_Jun,  SMI_Aug, SMI_Oct, P_May_demeaned:T_Oct_demeaned)
x

## only include main effects select by EARTH without interaction ##
x_select <- Maize_meteo_select %>% select(SMI_Jun,  P_Jun_demeaned:P_Aug_demeaned, T_May_demeaned:T_Oct_demeaned)
x_select
...
####################################
#### Data with T instead of PET ####
x_PET <- Maize_meteo_select %>% select(SMI_Jun,  SMI_Aug, SMI_Oct, P_May_demeaned:P_Oct_demeaned, PET_May_demeaned:PET_Oct_demeaned)
x_PET

## only include main effects select by EARTH without interaction ##
x_PET_select <- Maize_meteo_select %>% select(SMI_Jun,  SMI_Aug, P_May_demeaned:P_Sep_demeaned, PET_May_demeaned, PET_Jul_demeaned:PET_Oct_demeaned)
x_PET_select

###########################################################################
#### Use of SMI for May, Jul, and Sep instead of June, August, and Oct ####
# x_SMI <- Maize_meteo_select %>% select( SMI_May, SMI_Jul , SMI_Sep, P_May_demeaned:T_Oct_demeaned)
# x
...
## Explained Variables ##
y <- Maize_meteo %>% select(siloMaizeAnomaly)
...
data_select_outlier <- data_select[-c(984,957,365), ]
...
data_PET_select_outlier <- data_PET_select[-c(984,957,365), ]
...
############################
#### Define tuning grid ####
gbmGrid_noInteraction <-  expand.grid(nprune=c(1:22), degree=c(1))

###########################################
#### Model which use T instead of PET ####
model_earth_noInteraction_T <-  train(siloMaizeAnomaly ~ ., 
                                    data=data, 
...
## Check summaries of models without interaction ##
summary(model_earth_noInteraction_T)
summary(model_earth_noInteraction_PET $finalModel)

#### Derive Variable importance list ####
varImp(model_earth_noInteraction_T )
varImp(model_earth_noInteraction_PET )
# evimp(model_earth_noInteraction_PET$finalModel)
# evimp(model_earth_noInteraction_T$finalModel)

#### Look at plotmo for models without interaction #####
plotmo(model_earth_noInteraction_PET$finalModel)
plotmo(model_earth_noInteraction_T$finalModel)
...
##############################################################################
gbmGrid <- expand.grid(nprune=c(1:22), degree=c(2))
...
' Further, I only employ interactions to the main terms found in the step before. '



## Temperature Data ##
model_earth_preselect_T <-  train(siloMaizeAnomaly ~ ., 
                      data=data_select, 
                      trControl=train_control, 
                      method="earth" , 
                      # preProcess=c("center", "scale"),
...
## PET data ##
model_earth_preselect_PET <-  train(siloMaizeAnomaly ~ .,
...
model_earth_preselect_PET

## Check summaries of models without interaction ##
summary(model_earth_preselect_T)
summary(model_earth_preselect_PET $finalModel)

#### Derive Variable importance list ####
varImp(model_earth_preselect_T )
varImp(model_earth_preselect_PET )
# evimp(model_earth_preselect_PET$finalModel)
# evimp(model_earth_preselect_T$finalModel)

#### Look at plotmo for models without interaction #####
plotmo(model_earth_preselect_PET$finalModel)
plotmo(model_earth_preselect_T$finalModel)
...
resamps <- resamples(list(model_earth_noInteraction_PET = model_earth_noInteraction_PET,
                          model_earth_preselect_PET     = model_earth_preselect_PET, 
                          model_earth_noInteraction_T   = model_earth_noInteraction_T,     
                          model_earth_preselect_T       = model_earth_preselect_T
                              ) )
summary(resamps)
str(summary_resamps_EARTH)
diffs <- diff(resamps)
summary(diffs)
...
###########################
#### Comparative Plots ####
plot.earth.models(list(model_earth_noInteraction_PET$finalModel,
                       model_earth_preselect_PET$finalModel,
                       model_earth_noInteraction_T$finalModel, 
                       model_earth_preselect_T$finalModel))

###########################################
#### Plots to check model performancen ####
plot(model_earth_noInteraction_PET$finalModel, main="model_earth_noInteraction_PET")
plot(model_earth_preselect_PET$finalModel, main="model_earth_preselect_PET")
plot(model_earth_noInteraction_T$finalModel, main="model_earth_noInteraction_T")
plot(model_earth_preselect_T$finalModel, main="model_earth_preselect_T")

# Outlier 984,957,365 in allen vier Modellen in der ersten Runde
data_PET_select[c(984,957,365),] 
data_select[c(984,957,365),] 

# Outlier 165,975,958 in allen vier Modellen in der zweiten Runde #
data_PET_select_outlier[c(1265,975,958),] 
' In dieser Runde scheint es nur noch Sinn zu machen, '




predict(model_earth_noInteraction_T, newdata=  Climate_modelmatrix)
...
# model_mgcv_bestEARTH_T_interaction_ti_PET_k5 <-  gam(siloMaizeAnomaly ~ 
#                                                       ti(SMI_Jun, k=5) + 
#                                                       ti(SMI_Aug, k=5)  + 
...
#                                             data=data_PET_select)  
# 
# model_mgcv_bestEARTH_T_interaction_ti_PET_k4 <-  gam(siloMaizeAnomaly ~ 
#                                                      ti(SMI_Jun, k=4) + 
#                                                      ti(SMI_Aug, k=4)  + 
...
model_mgcv_bestEARTH_T_interaction_ti_PET_k3 <-  gam(siloMaizeAnomaly ~ 
                                                     ti(SMI_Jun, k=3) + 
                                                     ti(SMI_Aug, k=3)  + 
...
# model_mgcv_bestEARTH_T_interaction_ti_PET_PETinteractionRestricted <-  gam(siloMaizeAnomaly ~ 
#                                                   ti(SMI_Jun) + 
#                                                   ti(SMI_Aug)  + 
#                                                   ti(PET_May_demeaned) +
#                                                   ti(P_Jun_demeaned) +
...
# model_mgcv_bestEARTH_T_interaction_ti_PET <-  gam(siloMaizeAnomaly ~ 
#                                                                            ti(SMI_Jun) + 
#                                                                            ti(SMI_Aug)  + 
#                                                                            ti(PET_May_demeaned) +
#                                                                            ti(P_Jun_demeaned) +
...
plot(model_mgcv_bestEARTH_interaction_ti_PET_k3)
...
' Okay, die vis.gam plots sehen wie die plotmo plots aus. Daher werde ich letzere bei der Interpretation benutzen. '

# persp(model_mgcv_bestEARTH_interaction_ti_PET_k3)
...
gam.check(model_mgcv_bestEARTH_interaction_ti_PET_k3)


####################################################################################
#### Models employing T instead of PET - needs to be updated for EARTH results ####
##################################################################################

################################
#### model with interaction ####
# model_mgcv_bestEARTH_T_interaction_ti <-  gam(siloMaizeAnomaly ~
#                                               ti(SMI_Jun) + 
#                                               ti(P_Jun_demeaned)  +
#                                               ti(P_Jul_demeaned) +
#                                               ti(T_Jul_demeaned) +
...
model_mgcv_bestEARTH_interaction_ti_k3_T <-  gam(siloMaizeAnomaly ~
                                              ti(SMI_Jun, k=3) + 
                                              ti(P_Jun_demeaned, k=3)  +
                                              ti(P_Jul_demeaned, k=3) +
                                              ti(T_Jul_demeaned, k=3) +
                                              ti(T_Aug_demeaned, k=3) +
                                              ti(P_Jul_demeaned, P_Aug_demeaned, k=3) +
                                              ti(T_May_demeaned, T_Jul_demeaned, k=3) +
                                              ti(T_Jul_demeaned, T_Aug_demeaned, k=3) +
                                              ti(T_Jul_demeaned, T_Oct_demeaned, k=3) +
                                              ti(T_Aug_demeaned, T_Oct_demeaned, k=3),
                                            # select=TRUE,
                                            data= Maize_meteo)

# model_mgcv_bestEARTH_T_interaction_ti_k4 <-  gam(siloMaizeAnomaly ~
#                                                  ti(SMI_Jun, k=4) + 
#                                                  ti(P_Jun_demeaned, k=4)  +
#                                                  ti(P_Jul_demeaned, k=4) +
#                                                  ti(T_Jul_demeaned, k=4) +
#                                                  ti(T_Aug_demeaned, k=4) +
...
# model_mgcv_bestEARTH_noInteraction_k3_T <-  gam(siloMaizeAnomaly ~ 
#                                ti(SMI_Jun, k=3)  +  
#                                ti(P_Jun_demeaned, k=3) + 
#                                ti(P_Jul_demeaned, k=3) + 
#                                ti(P_Aug_demeaned, k=3) + 
#                                ti(T_May_demeaned, k=3) + 
#                                ti(T_Jun_demeaned, k=3) + 
#                                ti(T_Jul_demeaned, k=3) + 
#                                ti(T_Aug_demeaned, k=3) +
#                                ti(T_Sep_demeaned, k=3) + 
#                                ti(T_Oct_demeaned, k=3),
#                              data= Maize_meteo) 

model_mgcv_bestEARTH_noInteraction_k4_T <-  gam(siloMaizeAnomaly ~ 
                                  ti(SMI_Jun, k=4)  +  
                                  ti(P_Jun_demeaned, k=4) + 
                                  ti(P_Jul_demeaned, k=4) + 
                                  ti(P_Aug_demeaned, k=4) + 
                                  ti(T_May_demeaned, k=4) + 
                                  ti(T_Jun_demeaned, k=4) + 
                                  ti(T_Jul_demeaned, k=4) + 
                                  ti(T_Aug_demeaned, k=4) +
                                  ti(T_Sep_demeaned, k=4) + 
                                  ti(T_Oct_demeaned, k=4),
                                data= Maize_meteo)


#### Show function plots of MGCV models ####
plotmo(model_mgcv_bestEARTH_noInteraction_T, level = 0.95 )
plotmo(model_mgcv_bestEARTH_noInteraction_k3_T, level = 0.95 )
plotmo(model_mgcv_bestEARTH_noInteraction_k4_T, level = 0.95 )
plotmo(model_mgcv_bestEARTH_interaction_ti, level = 0.95 )
plotmo(model_mgcv_bestEARTH_interaction_ti_k3_T, level = 0.95 )
plotmo(model_mgcv_bestEARTH_interaction_ti_k4_T, level = 0.95 )

#### Export most relevant plotmo ####
pdf("./figures/figures_exploratory/Train/Caret/GAM/MCGV_bestEARTH_T.pdf", height=8.27, width=11.69)
plotmo(model_mgcv_bestEARTH_T, level = 0.95 )
dev.off()

pdf("./figures/figures_exploratory/Train/Caret/GAM/MCGV_bestEARTH_interaction_T_k3.pdf", height=8.27, width=11.69)
plotmo(model_mgcv_bestEARTH_interaction_ti_k3_T, level = 0.95 )
dev.off()

#### Summaries of MGCV models ####
summary(model_mgcv_bestEARTH_noInteraction_T)
summary(model_mgcv_bestEARTH_noInteraction_k3_T)
summary(model_mgcv_bestEARTH_noInteraction_k4_T )
summary(model_mgcv_bestEARTH_interaction_ti )
summary(model_mgcv_bestEARTH_interaction_ti_k3_T )
summary(model_mgcv_bestEARTH_interaction_ti_k4_T )

#### gamChecks plotd for models ####
par(mfrow=c(2,2))
gam.check(model_mgcv_bestEARTH_T )
gam.check(model_mgcv_bestEARTH_noInteraction_k3_T)
gam.check(model_mgcv_bestEARTH_interaction_ti)
gam.check(model_mgcv_bestEARTH_interaction_ti_k3_T)
gam.check(model_mgcv_bestEARTH_interaction_ti_k4_T)

#### Export most relevant plotmo ####
pdf("./figures/figures_exploratory/Train/Caret/GAM/gamCheck_MCGV_bestEARTH_T.pdf", height=8.27, width=11.69)
gam.check(model_mgcv_bestEARTH_T )
dev.off()

pdf("./figures/figures_exploratory/Train/Caret/GAM/gamCheck_MCGV_bestEARTH_interaction_T_k3.pdf", height=8.27, width=11.69)
gam.check(model_mgcv_bestEARTH_interaction_ti_k3_T)
dev.off()



' te appears to be less wiggely compared to splines. Further, I think it is better suited that splines for interaction.'
...
# 
# summary(model_mgcv_bestEARTH_T_interaction)
# summary(model_mgcv_bestEARTH_interaction_ti_k4_T)
# summary(model_mgcv_bestEARTH_interaction_ti_k5)
# 
# model_mgcv_bestEARTH_T_interaction$method
# par(mfrow=c(4,4))
# plot.gam(model_mgcv_bestEARTH_T_interaction, scheme=c(2))
# plot.gam(model_mgcv_bestEARTH_T_interaction_k, scheme=c(2))
# plot.gam(model_mgcv_bestEARTH_interaction_ti_k4_T, scheme=c(2))
# plot.gam(model_mgcv_bestEARTH_interaction_ti_k5, scheme=c(2))
# 
# plot.gam(model_mgcv_bestEARTH_T_interaction, scheme=c(1))
# plot.gam(model_mgcv_bestEARTH_T_interaction_k, scheme=c(1))
# plot.gam(model_mgcv_bestEARTH_interaction_ti_k4_T, scheme=c(1))
# plot.gam(model_mgcv_bestEARTH_interaction_ti_k5, scheme=c(1))
# 
# plotmo(model_mgcv_bestEARTH_T_interaction,  level=.95)
# plotmo(model_mgcv_bestEARTH_T_interaction_k, level=.95)
# plotmo(model_mgcv_bestEARTH_interaction_ti_k4_T, level=.95)
...
#######################################################
#### Spline version of model heurisitcally derived ####
...
## Export plotmo and gamc.heck ##
pdf("./figures/figures_exploratory/Train/Caret/GAM/MGCV_Base_JunAugSMI.pdf", height=8.27, width=11.69)
par(mfrow=c(2,2))
plotmo(model_mgcv_BASE , level = 0.95, ylim =c(-200, 150), main = "MGCV_Base_JunAugSMI")
dev.off()

pdf("./figures/figures_exploratory/Train/Caret/GAM/gamCheck_MGCV_Base_JunAugSMI.pdf", height=8.27, width=11.69)
par(mfrow=c(2,2))
gam.check(model_mgcv_BASE)
dev.off()
...
# # 
# # Climate <-  read_csv( pasti("./data/data_proj/", "MeteoMonth_df_tidy_", namelist_RCMs[[1]],"_19712000_demean.csv", sep=""))
# # 
