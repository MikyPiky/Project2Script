...
for (s in 1:length(modelListMatrixNames)){
  ...
  for (t in 1:5){
    ...
    ##############################################################################################################################################################################
    #### Plot of Maximal and Minimal Values ####
    ##############################################################################################################################################################################
    summary((PredictData_df_tidy_summaries_sf_list[[1]]))
    summary((PredictData_df_tidy_summaries_sf_list[[2]]))
...
for (s in 1:length(modelListMatrixNames)){
  ...
  for (t in 1:5){
    ...
    ################################################################################################################
    #### Make list of years ####
    years_list <- seq(1951, 2099)
    
    for (r in 1:length(years_list)){
      PredictData_df_tidy_annual   <-
        PredictData_df_tidy  %>%
        filter(model == namelist_models[[t]] )    %>%
        filter(year == years_list[[r]])

      #### Merge with Spatial Information ####
      PredictData_df_tidy_annual_sf <- merge(vg2500_krs, PredictData_df_tidy_annual, by.x = "RS", by.y = "comId", all.x=T, sort=T)
      str(PredictData_df_tidy_annual_sf)
      
      
      #### Plot yield ####
      summary(PredictData_df_tidy_annual_sf$Y)
      myPalette <- colorRampPalette((brewer.pal(11, "YlGn")))
      sc <- scale_fill_gradientn("Yield", colours = myPalette(100), limits=c(200, 600))
      
       plot_annual_Y <-
        ggplot(PredictData_df_tidy_annual_sf ) +
        geom_sf(aes(fill = Y)) +
        ggtitle(paste( modelListYieldNames[[s]],", Year: ", years_list[r], sep="")) + sc +
        theme_bw() +        theme(plot.title = element_text(hjust = 0.5)) 
      
      ggsave(paste("./figures/figures_exploratory/Proj/", modelListMatrixNames[[s]],"/Annual/plot_annual_yield_", years_list[r], "_", namelist_models[[t]],".pdf", sep=""), plot = plot_annual_Y , width=14, height=8)
      
      #### Plot yield ####
      summary(PredictData_df_tidy_annual_sf$Y_anomaly)
      myPalette <- colorRampPalette((brewer.pal(11, "BrBG")))
      sc <- scale_fill_gradientn("Yield Anomaly", colours = myPalette(100), limits=c(-300, 300))
      
      plot_annual_YAnomaly <-
        ggplot(PredictData_df_tidy_annual_sf ) +
        geom_sf(aes(fill = Y_anomaly)) +
        ggtitle(paste( modelListYieldNames[[s]],", Year: ", years_list[r], sep="")) + sc +
        theme_bw() +        theme(plot.title = element_text(hjust = 0.5)) 
      
      ggsave(paste("./figures/figures_exploratory/Proj/", modelListMatrixNames[[s]],"/Annual/plot_annual_yieldAnomaly_", years_list[r], "_",namelist_models[[t]],".pdf", sep=""), plot = plot_annual_YAnomaly , width=14, height=8)
